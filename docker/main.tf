terraform {
  backend "s3" {
    bucket         = "edsbucket1"
    key            = "terraform/terraform.docker.tfstate"
    region         = "us-east-1"
    encrypt        = true
  }
...
provider "proxmox" {
  ...
  pm_debug = true
  pm_log_enable = true
  pm_log_file   = "log/terraform-docker-proxmox.log"
  pm_log_levels = {
    _default = "info"
    _capturelog = ""
  }
...
resource "proxmox_vm_qemu" "docker" {
  ...
  name = "docker-host-${each.value}"
  #agent = 1
  target_node = "hera"
  vmid = 0
  desc = "docker-host-${each.value}"
  clone = var.docker_clone_name
